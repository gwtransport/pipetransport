...
``cout`` by the node the measurement was taken at --- and the arrays inside them are positional
on ``tedges``. There is no index for the package to
...
- **Coarsening is an uncontrolled approximation.** Averaging a finer concentration onto the shared
  ...
  hour, modelled against its hourly mean, delivers water of a visibly different age, because the
  mean flow never reproduces the arrival times the burst does.
...
network -- geometry, burial, wall -- of which there is one truth per network, validated when
...
constants are a scalar or a mapping by segment name. That is why ``length`` is a column of a
:class:`~pipetransport.network.PipeNetwork` while ``decay_rate`` is an argument of the pair.
...
The names match the transport pair, but neither direction takes an observation here, so
