...
def test_ipynb(ipynb_path):
    ...
    for src_line in body.splitlines():
        ...
        # Skip IPython magic commands and shell escapes. PythonExporter rewrites a magic such
        # as "%matplotlib inline" into a get_ipython() call, which raises NameError outside
        # IPython, so the converted form has to go as well. The safety header below already
        # forces the non-interactive backend that %matplotlib would have selected.
