...
def _get_all_code_snippets():
    ...
    # Scan RST files in docs/source, including the user_guide/, examples/ and api/ subdirectories
    for rst_file in sorted(docs_dir.rglob("*.rst")):
        ...
        for match in re.finditer(
            ...
            if code:
                ...
                snippets.append((code, line_num, str(rst_file.relative_to(docs_dir))))
...
def test_snippets_were_found():
    """Guard the harness: a regex that silently stops matching would make every snippet test vanish."""
    snippets = _get_all_code_snippets()
    sources = {fname for _, _, fname in snippets}
    assert "README.md" in sources, f"no Python snippet extracted from README.md, found only {sorted(sources)}"
    # The README documents the forward direction, the reverse direction, and decay/water age.
    assert sum(fname == "README.md" for _, _, fname in snippets) >= 3
